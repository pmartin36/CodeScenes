...
// in the floor — and AlignTo's no-target raycast/fallback-scan both search from the Crate's OWN
// current bounds toward a surface, so when the target surface ends up on the WRONG side of that
// (masked) starting position, neither can find it and the Crate is left embedded, never snapping.
// This means the bug's effect DOES survive to the final, fully-driven geometry (confirmed
// empirically, and why FloorBody below is a THICK floor, not a thin one — see its own comment) — so
// a plain "final observed geometry" assertion (no hand-positioned direct Evaluate(), no
// pre/post-drive bookkeeping) is the correct — and sufficient — regression check, matching the
// spec's own "Gate coverage" section. Authored with an explicit NodeHandle.None target (rather
// than a real anchor) so the no-target raycast/fallback-scan path — the one this regression is
// about — is the one actually exercised (a real target resolves by closed-form extent math and
// would not reproduce the bug's direction-sensitivity).
//
...
// and AlignTo's own [ExecuteAlways] OnValidate ALSO already self-drives synchronously inside
...
{body}
    ...
    // resting slightly low. AlignTo's downward raycast/fallback-scan both search from the object's
    ...
    // A Crate carrying an EXPLICIT authored transform ABOVE the floor PLUS FitSize + AlignTo on
    ...
    // volume (floor spans y in [-2, 2]) — so the Crate starts fully EMBEDDED; AlignTo's downward
    ...
    // never snaps and the Crate is left embedded. NodeHandle.None keeps this on the no-target
    // raycast/scan path (an explicit target would resolve by closed-form math regardless of self's
    // current position, and would not reproduce the bug).
    ...
    // PlanExecutor adds the FitSize/AlignTo MonoBehaviour (triggering its [ExecuteAlways]
    ...
    // cached for the rest of the domain's lifetime. For FitSize/AlignTo this fires exactly ONCE
    ...
    [Test]
    public void Build_AuthoredTransformAboveFloorWithFitSizeAndAlignTo_DrivesToFlushRest()
    {
        ...
        // Drive the real component path (execution order: FitSize(-100) then AlignTo(-90)) —
        ...
        // the Crate starts embedded deep inside the (thick) floor and neither AlignTo's downward
        ...
        // the fix, the authored Y materializes in full (well above the floor), so AlignTo resolves
