...
// M-Spatial (Sizer/Snapper) full bidirectional SYNC round-trip gate tests — the spec's confirmation
...
{body}
    ...
    // distinguishable from any driven-axis value used below). No Sizer/Snapper — a plain surface.
    ...
    // A Crate: unit Cube, aspect-locked Sizer height 2, down-Snapper, starting above the floor.
    ...
    // PlanExecutor adds the Sizer/Snapper MonoBehaviour (triggering its [ExecuteAlways] OnValidate ->
    ...
    // creates a Sizer — irrelevant here (the reporting surface is the assertions below, not the
    ...
    // Sizer/Snapper (both [ExecuteAlways] with an OnValidate that logs a located error against that
    // bare probe object) this fires exactly ONCE per type, non-deterministically on whichever [Test]
    // happens to sync a Sizer/Snapper first. Force the warm-up HERE, isolated from any real test's
    ...
    public void TearDown()
    {
        ...
        // LATER suite's Snapper fallback-scan (a global "every Renderer in the scene" search) —
        // RoundTripSpatialTests.cs's tests build directly against the ambient scene without ever
        // resetting it themselves.
        EditorSceneManager.NewScene(NewSceneSetup.EmptyScene, NewSceneMode.Single);
    }

    // 11. Anti-churn (source pin): author Sizer+Snapper, Build, Sync with NO edit. The rewritten
    //     source still carries .Sizer/.Snapper and carries NO .Transform(scale:/pos:) — the driven
    //     channels never leak into source — and the sync itself is a no-op.
    [Test]
    public void RoundTrip_SizerSnapperNoEdit_SyncIsNoOpAndEmitsNoTransform()
    {
        ...
        crate.GetComponent<Sizer>().Evaluate();
        crate.GetComponent<Snapper>().Evaluate();

        var result = EmittedCodeCompiles.SyncAndAssertCompiles(_builderPath, _sidecarPath, EditorSceneManager.GetActiveScene());
        Assert.IsFalse(result.Changed, "An unedited Sizer/Snapper scene must Sync as a no-op.");

        var rewritten = File.ReadAllText(_builderPath);
        StringAssert.Contains(".Sizer(", rewritten, "Rewritten source must still carry the .Sizer(...) call.\n" + rewritten);
        StringAssert.Contains(".Snapper(", rewritten, "Rewritten source must still carry the .Snapper(...) call.\n" + rewritten);
    ...
    public void SceneToCode_DraggedSnappedAxis_ReSnapsY_FreeXPersists()
    {
        ...
        var crate = FindRoot(EditorSceneManager.GetActiveScene(), "Crate");
        var sizer = crate.GetComponent<Sizer>();
        var snapper = crate.GetComponent<Snapper>();
        sizer.Evaluate();
        ...
        // survive, and the Snapper must still correctly re-snap Y regardless of the source's
        ...
        rebuiltCrate.GetComponent<Sizer>().Evaluate();
        var rebuiltSnapper = rebuiltCrate.GetComponent<Snapper>();
        rebuiltSnapper.Evaluate();
        var rebuiltBounds = rebuiltCrate.GetComponent<Renderer>().bounds;
        Assert.AreEqual(0.5f, rebuiltBounds.min.y, Tol, "The rebuilt Snapper must still re-snap Y flush on the floor.");
    }

    // 13. Field edit round-trips: editing the Sizer height and toggling a Snapper flag directly on
    //     the live components must patch ONLY the corresponding .Sizer/.Snapper argument — nothing
    //     structural (no .Transform) — and a second Sync (no further edit) is a no-op.
    [Test]
    public void SceneToCode_EditedSizerHeightAndSnapperFlag_PatchesArgumentOnly()
    {
        ...
        var crate = FindRoot(EditorSceneManager.GetActiveScene(), "Crate");
        Assert.IsNotNull(crate, "Crate was not created by SceneBuilderBuild.Run");
        crate.GetComponent<Sizer>().height = 3f;
        crate.GetComponent<Snapper>().left = true;

        var result = EmittedCodeCompiles.SyncAndAssertCompiles(_builderPath, _sidecarPath, EditorSceneManager.GetActiveScene());
        Assert.IsTrue(result.Changed, "Editing Sizer.height and Snapper.left is a real scene change; Sync must report Changed.");

        var rewritten = File.ReadAllText(_builderPath);
        StringAssert.Contains(".Sizer(height: 3f)", rewritten, "Sizer.height edit must patch the .Sizer(height:) argument.\n" + rewritten);
        StringAssert.DoesNotContain("height: 2f", rewritten, "The old Sizer height argument must not remain.\n" + rewritten);
        StringAssert.Contains("left: true", rewritten, "Snapper.left toggle must patch the .Snapper(...) call to include left: true.\n" + rewritten);
        ...
        StringAssert.Contains(".Transform(pos: (0f, 5f, 0f))", rewritten,
            "The Crate's pre-existing, unrelated pos: argument must remain unchanged by a pure Sizer/Snapper field edit.\n" + rewritten);
    ...
    // 14. Created-in-editor object with a Snapper: a GameObject added directly in the scene (not via
    //     the builder) with a Snapper attached must append a new .Add(...) statement carrying
    //     .Snapper(down: true) on scene->code sync, and the emitted source must compile. A second
    //     Sync (no further change) is a no-op.
    [Test]
    public void SceneToCode_CreatedObjectWithSnapper_AppendsSnapperCall_SecondSyncNoOp()
    {
        ...
        var snapper = created.AddComponent<Snapper>();
        snapper.down = true;
        ...
        StringAssert.Contains(".Snapper(down: true)", rewritten, "The created object's Snapper must emit the dedicated .Snapper(down: true) call.\n" + rewritten);
        StringAssert.DoesNotContain("Component<SceneBuilder.Authoring.Snapper>", rewritten,
            "Snapper must never be emitted via the generic .Component<>() form.\n" + rewritten);
    ...
    // 18. Sizer back-solves intent from a manual scale: two Evaluate() calls are mandatory — the
    //     manual-override branch only fires when the component has already written once and localScale
    //     has since diverged. After the manual override, Sync must patch the Sizer's height argument to
    //     the back-solved world height and never write a raw .Transform(scale:).
    [Test]
    public void SceneToCode_ManualScaleOnSizer_BackSolvesHeightIntent_NoTransformWrite()
    {
        const string body =
            "        var crate = scene.Add(\"Crate\")\n" +
            "            .Component<UnityEngine.MeshFilter>(c => c.Set(\"m_Mesh\", Builtin(\"Cube\")))\n" +
        ...
        var rewritten = File.ReadAllText(_builderPath);
        StringAssert.Contains(".Sizer(height: 4f)", rewritten, "Sync must patch the Sizer height argument to the back-solved world height.\n" + rewritten);
        StringAssert.DoesNotContain("height: 2f", rewritten, "The old Sizer height argument must not remain.\n" + rewritten);
    ...
    // 19 (sync half). Disabling a Sizer releases its driven channel: a manual scale set while disabled
    //    is no longer suppressed and DOES sync as a .Transform(scale:) write.
    [Test]
    public void SceneToCode_DisabledSizer_ReleasesChannel_ManualScaleSyncs()
    {
        const string body =
            "        var crate = scene.Add(\"Crate\")\n" +
            "            .Component<UnityEngine.MeshFilter>(c => c.Set(\"m_Mesh\", Builtin(\"Cube\")))\n" +
        ...
        var result = EmittedCodeCompiles.SyncAndAssertCompiles(_builderPath, _sidecarPath, EditorSceneManager.GetActiveScene());
        Assert.IsTrue(result.Changed, "A disabled Sizer's released channel must let the manual scale sync as a real change.");
        ...
        // .Sizer/.Snapper renderer's hand-formatted "name: value" — assert on structure, not on the
        ...
        StringAssert.IsMatch(@"\.Transform\(\s*scale:\s*\(3f,\s*3f,\s*3f\)\s*\)", rewritten,
            "A disabled Sizer must release the scale channel so the manual scale is written as a .Transform(scale:).\n" + rewritten);
    }
}
