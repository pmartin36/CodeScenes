# M-Spatial — Spatial authoring components (FitSize + SurfaceSnap, editor-time live constraints)

### Additions to the contract

**Two editor-only components + one suppression concept — no new `ValueNode` kind.** This milestone adds
two live-evaluating **editor-time components** (`FitSize`, `SurfaceSnap`) that let an author express spatial
...
|---|---|---|
| `FitSize` (runtime-assembly `[ExecuteAlways]` MonoBehaviour) | drives **local scale** so the mesh hits an author-declared **world-space** size; stripped from player builds | M-Spatial |
| `SurfaceSnap` (runtime-assembly `[ExecuteAlways]` MonoBehaviour) | drives **local position** on chosen axes so the mesh's real bounds rest against a surface; stripped from player builds | M-Spatial |
...
| `.FitSize(...)` / `.SurfaceSnap(...)` authoring sugar + dedicated parse/emit arms | fluent node-handle methods (siblings of `.Transform(…)`), lowering to an M3 component-with-fields | M-Spatial |

`DrivenChannels` is **re-derived on each side, never persisted** (the sidecar stays untouched, exactly
as §17 re-derives a built-in name live) — the expected model derives it from the `.FitSize/.SurfaceSnap`
...
> (`specs/06`)**. The one place this milestone brushes M5 (SurfaceSnap's optional explicit-target
...
scene.Add("Crate")
     ...
     .FitSize(height: 1.2f)                                              // 1.2 world units tall, aspect kept
     .SurfaceSnap(down: true);                                            // its real bottom rests on the floor
```
...
FitSize solves (1); SurfaceSnap solves (2) and (3). Both must be **live** (constraint 2/3 have moving inputs)
...
- **`FitSize`** — declare a mesh's target **world-space** size, indifferent to native size. Two authoring
  forms:
...
- **`SurfaceSnap`** — snap a mesh to a surface, **origin-agnostic**: uses the mesh's actual **world
...
- **Snap target = raycast scene geometry, no wiring** (default), **with an optional explicit target
  ...
  raycast.
- **SurfaceSnap runs AFTER FitSize.** It needs the final, post-sizing world size to know where the object's
  real face is. This ordering is enforced at evaluate time (`[DefaultExecutionOrder]`) and preserved in
  emitted source (FitSize call before SurfaceSnap call).
...
- **Driven-channel suppression (both directions).** The **raw** transform channels the components
  derive — FitSize → local scale; SurfaceSnap → local position on the snapped axes (X/Y/Z per the set
  faces) — are **driven**: code holds the intent (`.FitSize(...)` / `.SurfaceSnap(...)`) and **never** the
...
- **Both-direction round-trip of intent.** Author `.FitSize/.SurfaceSnap` in code → components applied in
...
- **Collision resolution between multiple snapped objects.** Each object snaps to scene surfaces
  independently; two SurfaceSnap objects are not de-conflicted against each other (no mutual pushout, no
  stacking solver).
- **Non-mesh objects.** A GameObject with no `MeshFilter`+mesh / no `Renderer` (empties, lights,
  cameras, bare `RectTransform` UI) has no bounds to read → out of scope; `.FitSize/.SurfaceSnap` on such a
  node is a located error, not a guess. (UI layout is §13's job.)
- **Rotation authoring / surface-normal alignment.** Neither component drives rotation, and SurfaceSnap
...
- **`TransformData.DrivenChannels : ChannelMask`** — `None` by default, so every existing node is
  ...
  - a node carrying a `FitSize` ⇒ `|= Scale`;
  - a node carrying a `SurfaceSnap` with `left`/`right` set ⇒ `|= PositionX`;
  - a node carrying a `SurfaceSnap` with `up`/`down` set ⇒ `|= PositionY`;
  - a node carrying a `SurfaceSnap` with `forward`/`back` set ⇒ `|= PositionZ`.
  (`ChannelMask.PositionZ` already exists in the enum above — depth adds no new mask member, only a new
  derivation source.)
- **`FitSize` / `SurfaceSnap` as M3 component data.** In the `SceneModel` these are ordinary components in the
  ...
  The FitSize field map is one of `{ width|height|depth : float }` (aspect-locked) or `{ size : Vec3 }`
  (explicit); the SurfaceSnap field map is `{ up, down, left, right, forward, back : bool, target : ObjectRef? }`.
  ...
  is not emitted into the `.FitSize(...)` / `.SurfaceSnap(...)` call.

**Functions/behaviors (each a testable contract)**

- **Parse `.FitSize(...)` / `.SurfaceSnap(...)`** — new arms in `BuilderParser`, matched exactly like the
  ...
  - `.FitSize(height: 2f)` → FitSize component `{ height: 2 }`, `DrivenChannels |= Scale`.
  - `.FitSize(size: (2,1,0.5f))` → FitSize component `{ size: (2,1,0.5) }`, `DrivenChannels |= Scale`.
  - `.FitSize(height: 2f, size: (…))` (aspect **and** explicit together) → **located error** (§7):
    ambiguous size authority, never a silent pick.
  - `.SurfaceSnap(down: true, left: true)` → SurfaceSnap `{ down:true, left:true }`, `DrivenChannels |=
    PositionX|PositionY`.
  - `.SurfaceSnap(down: true, back: true)` → SurfaceSnap `{ down:true, back:true }`, `DrivenChannels |=
    PositionY|PositionZ`.
  - `.SurfaceSnap(left: true, right: true)` (or `up`+`down`, or `forward`+`back`) → **located error**:
    contradictory axis.
  - `.SurfaceSnap(down: true, target: floor)` → additionally an `ObjectRef` to the `floor` handle's
...
- **Driven-channel suppression in `Diff` (both directions), the load-bearing behavior.** For a node
  ...
  - FitSize node: a snapshot `m_LocalScale` differing from the model produces **no** scale op.
  - SurfaceSnap-down node: a snapshot `m_LocalPosition.y` drift produces **no** position-Y op; `.x`/`.z`
    still diff normally (the free axes).
  - The **component fields themselves** (FitSize `height`, SurfaceSnap flags/target) diff **normally** via M3
...
- **Emit `.FitSize(...)` / `.SurfaceSnap(...)`** — dedicated `SourceExpr`/emit arms producing the fluent form
  (**not** the generic `.Component<FitSize>(c => c.Set(...))`), so scene→code reads as authored:
  - FitSize → `.FitSize(height: 2f)` / `.FitSize(size: (2f, 1f, 0.5f))` (floats f-suffixed via the shared
    `SourceExpr.Float`/`Vec3Literal`, invariant culture, exactly as §13).
  - SurfaceSnap → `.SurfaceSnap(down: true, left: true)` / `.SurfaceSnap(down: true, back: true)` (only the set
    flags emitted — `forward`/`back` alongside the horizontal/vertical flags) / `.SurfaceSnap(down: true,
    target: floor)`.
- **Deterministic ordering: FitSize before SurfaceSnap.** When both are emitted onto one node, the FitSize call
  precedes the SurfaceSnap call in source, mirroring the evaluate-time execution order; the ordering is
...
- **Reconcile (scene→code).** Reuses `ComponentReconciler` for the component fields, formatted through
  the dedicated emit:
  - an Inspector edit to a FitSize/SurfaceSnap field → a `PatchArgument` rewriting only that argument of the
    `.FitSize(...)` / `.SurfaceSnap(...)` call (formatting preserved, §5);
  - a FitSize/SurfaceSnap added to an editor-created object → the dedicated call appended onto that node's
...
- **`FitSize : MonoBehaviour` (runtime, `[ExecuteAlways]`, `[DefaultExecutionOrder(-100)]`).**
  ...
  - write `transform.localScale`. (A zero/degenerate bounds extent → no-op + one located warning, never
    a divide-by-zero.)
- **`SurfaceSnap : MonoBehaviour` (runtime, `[ExecuteAlways]`, `[DefaultExecutionOrder(-90)]` — after
  FitSize).** Serialized `bool up/down/left/right/forward/back` and an optional `Transform target`. Each
...
- **Driven-channel wiring into SceneBuilder sync.**
  - **Read (snapshot):** when the snapshot reader finds a live `FitSize`/`SurfaceSnap` on an object, it stamps
...
- **Build-strip via `IProcessSceneWithReport` (Editor assembly).** `OnProcessScene(Scene, BuildReport
  ...
  `FitSize`/`SurfaceSnap` in the scene, force one final `Evaluate()` (guarantees the baked transform is
...
- **Inspector "driven" affordance (enhancement, flagged).** To make "the component drives this
  ...
  `[ExecuteAlways]` re-evaluation (FitSize back-solves its intent from a manual scale; SurfaceSnap re-snaps a
...
```csharp
// FitSize — aspect-locked fit (the default form: exactly one driving dimension, aspect preserved)
public NodeHandle FitSize(float? width = null, float? height = null, float? depth = null);
// FitSize — explicit per-axis world size (non-uniform allowed)
public NodeHandle FitSize(Vector3 size);

// SurfaceSnap — one horizontal (left|right), one vertical (up|down), one depth (forward|back),
...
`width`/`height`/`depth` (aspect-locked) **or** `size:` (explicit) — never both, never none. SurfaceSnap
...
public class ArenaScene : ISceneDefinition {
    public void Build(SceneRoot scene) {
        var floor = scene.Add("Floor")
            ...
            .FitSize(size: (20, 1, 20));                       // 20×20 world units

        // A crate: 1.2 m tall (aspect kept), resting on whatever floor is below it.
        scene.Add("Crate")
             .Component<MeshFilter>(c => c.Set("m_Mesh", Builtin("Cube")))
             .FitSize(height: 1.2f)                            // FitSize first…
             .SurfaceSnap(down: true);                           // …SurfaceSnap second (needs the final size)
        ...
        scene.Add("Poster")
             ...
             .FitSize(width: 0.8f)
             .SurfaceSnap(down: true, left: true);
...
Lowering (Editor→Core): `.FitSize(...)` / `.SurfaceSnap(...)` → an M3 component-with-fields on the node **and**
`TransformData.DrivenChannels` set; base `.Transform(rot:/pos:)` may still be chained for the
non-driven channels (e.g. rotation, or a free axis SurfaceSnap doesn't touch).
...
1. `Parse_FitSizeHeight_YieldsFitSizeComponentAndDrivenScale` — `.FitSize(height: 2f)` → FitSize `{height:2}`,
   `DrivenChannels == Scale`.
2. `Parse_FitSizeExplicitSize_YieldsPerAxisFieldsAndDrivenScale` — `.FitSize(size:(2,1,0.5f))`.
3. `Parse_FitSizeAspectAndExplicitTogether_YieldsLocatedError` — both forms → located error, no throw.
4. `Parse_FitSizeNoDimension_YieldsLocatedError`.
5. `Parse_SurfaceSnapDownLeft_SetsFlagsAndDrivenPositionXY` — `DrivenChannels == PositionX|PositionY`.
6. `Parse_SurfaceSnapDownOnly_DrivesPositionYNotX` — `DrivenChannels == PositionY`.
7. `Parse_SurfaceSnapContradictoryAxis_YieldsLocatedError` — `left:true,right:true`, `up:true,down:true`,
   **and `forward:true,back:true`**.
8. `Parse_SurfaceSnapWithTarget_CarriesObjectRefToHandleLogicalId`.
9. `Parse_NodeWithoutSpatialComponent_DrivenChannelsNone`.
9b. `Parse_SurfaceSnapBack_DrivesPositionZ` — `.SurfaceSnap(back: true)` → SurfaceSnap `{back:true}`,
    `DrivenChannels == PositionZ`; `.SurfaceSnap(down:true, back:true)` → `DrivenChannels ==
    PositionY|PositionZ`.

**Diff / suppression** (the load-bearing pins)
10. `Diff_FitSizeNode_ScaleDriftInSnapshot_ProducesNoScaleOp`.
11. `Diff_SurfaceSnapDownNode_PositionYDrift_ProducesNoPositionYOp_ButXZStillDiff`.
11b. `Diff_SurfaceSnapBackNode_PositionZDrift_ProducesNoPositionZOp_ButXYStillDiff` — depth channel
    suppressed, the two free axes still diff.
12. `Diff_FitSizeFieldChanged_ProducesComponentFieldChange` — intent still diffs via M3.
13. `Diff_SurfaceSnapTargetRewired_ProducesComponentFieldChange`.
14. `Diff_NonDrivenNode_ScaleAndPositionStillDiffNormally` — regression: suppression is opt-in per channel.

**Emit + ordering**
15. `Emit_FitSize_EmitsDedicatedCallNotGenericComponent` — `.FitSize(height: 2f)`, not `.Component<FitSize>`.
16. `Emit_FitSizeExplicit_EmitsSizeVectorFSuffixed`.
17. `Emit_SurfaceSnap_EmitsOnlySetFlags` — `.SurfaceSnap(down: true, left: true)`.
18. `Emit_FitSizeBeforeSurfaceSnap_OrderingDeterministicAndStable` — both present → FitSize precedes SurfaceSnap;
    re-emit identical (no churn).
19. `Parse_Emit_SpatialCalls_TextRoundTripsIdentically`.

**Reconcile**
20. `Reconcile_EditedFitSizeHeightInScene_PatchesFitSizeArgumentOnly`.
...
22. `Reconcile_CreatedNodeWithSurfaceSnap_AppendsSurfaceSnapCall_SecondSyncNoOp` — §13 create-with-payload.
23. `Reconcile_SurfaceSnapAndFitSizeOnCreatedNode_AppendsInFitSizeThenSurfaceSnapOrder`.
24. `RoundTrip_FitSizeSurfaceSnap_Idempotent` — Materialize→parse→re-materialize and Reconcile→apply→
...
1. **FitSize hits an exact world size.** Author `.FitSize(height: 2f)` on a non-unit mesh; Build.
   *Expected:* the object's `Renderer.bounds.size.y ≈ 2` (tolerance ~1e-3), width/depth scaled by the
   same factor (aspect preserved).
2. **FitSize under a scaled parent.** Parent `lossyScale = 3`; `.FitSize(height: 2f)` on the child.
   *Expected:* child still `bounds.size.y ≈ 2` world units (parent scale divided out).
3. **FitSize explicit per-axis.** `.FitSize(size: (2,1,0.5f))` on a Cube. *Expected:* bounds `≈ (2,1,0.5)`.
4. **SurfaceSnap down, three pivots.** Take the same mesh authored with pivot at feet, centre, and head;
   `.SurfaceSnap(down: true)` over a floor at `y = 0`. *Expected:* all three land with
   `Renderer.bounds.min.y ≈ floorTop` (origin-agnostic — the headline pivot test).
5. **SurfaceSnap up onto a ceiling.** `.SurfaceSnap(up: true)` under a ceiling. *Expected:* `bounds.max.y ≈
   ceilingBottom`.
6. **SurfaceSnap corner.** `.SurfaceSnap(down: true, left: true)` in a corner. *Expected:* `bounds.min.y ≈
   floorTop` **and** `bounds.min.x ≈ leftWallInner`.
7. **FitSize + SurfaceSnap together, ordering.** `.FitSize(height: 1.2f).SurfaceSnap(down: true)`. *Expected:* the
   object is 1.2 units tall **and** its (post-resize) bottom rests on the floor — proving SurfaceSnap read
   the final size, not the pre-FitSize size.
...
9. **Raycast fallback (no collider).** Snap onto a floor object that has a `Renderer` but **no
   ...
   lands.
10. **Explicit target override.** `.SurfaceSnap(up: true, target: ceiling)` where the raycast up would
    otherwise hit a different object first. *Expected:* it snaps to `ceiling`, not the raycast hit.
11. **Driven channels not emitted (the anti-churn pin).** Author `.FitSize(height: 2f).SurfaceSnap(down:
    true)`; Build; then Sync with no edit. *Expected:* source is unchanged — it contains `.FitSize(...)`
    and `.SurfaceSnap(...)` but **no** `.Transform(scale: …)` / `.Transform(pos: …)`; the Sync is a no-op.
...
13. **Field edit round-trips.** Change the `FitSize` height in the Inspector, or toggle a `SurfaceSnap` flag;
    Sync. *Expected:* the `.FitSize(height: …)` / `.SurfaceSnap(...)` argument updates; nothing else.
14. **Created-in-editor object with a SurfaceSnap.** Add a GameObject in the editor, add a `SurfaceSnap`
    component, set `down`; Sync. *Expected:* a `.Add("…")…​.SurfaceSnap(down: true)` statement appears
...
15. **Build-strip.** Build a player scene (or invoke the `IProcessSceneWithReport` on a scene copy with
    a non-null report). *Expected:* the built GameObject has **no** `FitSize`/`SurfaceSnap` component and **no
    missing script**, and its transform retains the baked size/position.
16. **Non-mesh node is refused.** `.FitSize(height: 2f)` on an empty GameObject (no `MeshFilter`).
    *Expected:* a located error naming the node — not a silent no-op or a divide-by-zero.
17. **Depth snap (forward/back).** `.SurfaceSnap(back: true)` against a back wall. *Expected:*
    `Renderer.bounds.max.z ≈ wallInner` (back face flush); `.SurfaceSnap(down: true, back: true)` ⇒ bottom on
    the floor **and** back flush to the wall (two-axis combine including depth).
18. **FitSize back-solves intent from a manual scale (§"Manual-override").** With `.FitSize(height: 2f)`
    active, hand-scale the object taller in the editor; let it tick, then Sync. *Expected:* the
    `.FitSize(height: …)` argument updates to the new world height (intent back-solved) — **not** a
    `.Transform(scale:)` write; a second Sync is a no-op.
19. **Enable-toggle releases the channel (§"Manual-override").** Disable the `FitSize`/`SurfaceSnap`
...
- **M3** (`specs/completed/04-m3-components-fields.md`) — **verified present**: `.Component<T>(c =>
  ...
  reconcile (`SceneBuilder.Core/Reconcile/ComponentReconciler.cs` + `ComponentPatchApplier.cs`). FitSize
  and SurfaceSnap **are** M3 components — authored, materialized and reconciled through this machinery; only
...
- **M4** (`specs/completed/05-m4-asset-references.md`) / **§17** — FitSize reads the node's mesh, assigned
...
- **M5 (`specs/06-m5-cross-object-references.md`, SHIPPED) — the SurfaceSnap `target:` override rides on
  ...
  override ships in its **full** form — the SurfaceSnap `target:` handle lowers to a `ValueNode.ObjectRef`
...
- **Manual-override = BACK-SOLVE INTENT WHILE ACTIVE (RATIFIED by the user, 2026-07-16).** When the
  ...
  **not** blindly revert and does **not** detach — it **conforms its own intent to the manual edit**:
  - **FitSize** (a driven channel with a scalar intent): a manual `localScale` change alters the world
    size; the FitSize reads the new world bounds size and **back-solves** its intent field
    (`height`/`width`/`depth`, or the per-axis `size`) to match, so the manual change is preserved **as
    intent** and round-trips to source as a `.FitSize(...)` **argument** patch (via the component-field
  ...
  - **SurfaceSnap** (a boolean, flush-to-surface intent with **no scalar to back-solve**): a manual move
    ...
    **free** axis (any axis the SurfaceSnap does not drive) moves normally and syncs. To relocate a snapped
    object the author moves the surface, retargets, or disables the component.
  - **Per-component enable toggle (the escape hatch):** disabling the `FitSize`/`SurfaceSnap` (its standard
  ...
  - **Mechanism the pipeline must build:** the component distinguishes a **user** edit from its **own**
    ...
    divergence as a user edit (FitSize → back-solve; SurfaceSnap snapped axis → re-snap). And
...
- **`[ExecuteAlways]` per-frame raycast cost.** A scene of many SurfaceSnaps each casting every editor frame
...
- **Build-strip correctness depends on baked values being current.** The `IProcessSceneWithReport`
  ...
  cannot ship a wrong transform. If a FitSize/SurfaceSnap input (mesh/surface) is itself missing at build
...
- **Raycast needs colliders; the bounds fallback is approximate.** A collider gives an exact surface; the
  ...
  out of scope.
- **Multiple snapped objects do not de-collide** (out of scope) — two SurfaceSnap objects can overlap;
...
- **Component final names — RATIFIED: `FitSize` and `SurfaceSnap`** (user 2026-07-16). Verb-free nouns,
